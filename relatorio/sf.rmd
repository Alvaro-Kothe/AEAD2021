...
```{r rep-lag, fig.cap="Gráfico da resposta contra a sua defasagem."}
...
corrplot.mixed(corplot, tl.pos = "lt", tl.col = "black",
               number.cex = .6)
```

```{r eval=FALSE, include=FALSE}
estacoes %>% filter(estacao_codigo %in% colnames(corplot)[1:6]) %>%
    head()
```
Todas as bacias que apresentaram correlação acima de 0.9 pertencem ao rio são francisco.
